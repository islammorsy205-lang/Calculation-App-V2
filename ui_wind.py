    ...
    with st.container(border=True):
        ...
        if st.toggle("Include Detailed Strut & Anchor Checks", value=bool(get_val("inc_tilt", i, False)), key=f"inc_tilt_{i}"):
            ...
            with ts_1:
                ...
                with col_g1:
                    y1 = st.number_input("Y1 (m)", key=f"y1_{i}", step=0.05)
                    y2 = st.number_input("Y2 (m)", key=f"y2_{i}", step=0.05)
                    y3 = st.number_input("Y3 (m)", key=f"y3_{i}", step=0.05) if num_props == 3 else 0.0
                with col_g2:
                    x1 = st.number_input("X1 (m)", key=f"x1_{i}", step=0.05)
                    x2 = st.number_input("X2 (m)", key=f"x2_{i}", step=0.05)
                    x3 = st.number_input("X3 (m)", key=f"x3_{i}", step=0.05) if num_props == 3 else 0.0
